...
def start(argv):
    ...
    for opt, arg in opts:
        if opt == '-l':
            ...
            if engine not in ("google","googleCSE" , "linkedin", "pgp", "all", "google-profiles", "bing", "bingapi",
                              "yandex", "jigsaw", "dogpilesearch", "twitter", "googleplus", "yahoo", "baidu","411","zaba"):
                usage()
    ...
    if engine == "google":
        ...
        search = googlesearch.search_google("rob dan", 500, start)
        search.process()
    ...
    if engine == "exalead":
        ...
        print "[-] Searching in 411.."
        try:
            search = search411.search_411(word, limit);
        except Exception,e:
            print e;
        lst=search.process()
        print "List"
        ...
        print "[-] Searching in zaba.."
        try:
            search = searchzaba.search_zaba(word, limit);
        except Exception,e:
            print e;
        lst=search.process()
        print "List"
